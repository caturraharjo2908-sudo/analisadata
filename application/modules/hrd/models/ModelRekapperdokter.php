...
class Modelrekapperdokter extends CI_Model {
    ...
    public function datarincianpasien_bykeuepisode($dokter_idrajal, $startdaterajal, $endaterajal, $dokter_idranap, $startdateranap, $endateranap) {
        ...
        $query = "SELECT 'RAWAT JALAN' JENIS, DOKTER_ID, TO_CHAR(A.TGL_MASUK,'DD.MM.YYYY') PERIODE, COUNT(*) AS TOTAL_KUNJUNGAN
            ...
            AND A.TGL_MASUK <  TO_DATE(?, 'DD-MM-YYYY')
            --AND TO_CHAR(A.TGL_MASUK,'DD-MM-YYYY') = '2026'
            AND (
                    (
                        A.POLI_ID NOT IN (
                            'UGD01',
            ...
            AND  A.TGL_MASUK >= TO_DATE(?, 'DD-MM-YYYY')
            AND A.TGL_MASUK <  TO_DATE(?, 'DD-MM-YYYY')
            --AND TO_CHAR(A.TGL_MASUK,'YYYY') = '2026'

            GROUP BY DOKTER_ID, TO_CHAR(A.TGL_MASUK,'DD.MM.YYYY')";

        // Urutan binding: ID Dokter, Tanggal Awal, Tanggal Akhir (Rajal) -> ID Dokter, Tanggal Awal, Tanggal Akhir (Ranap)
    ...
    //         AND A.LAYAN_ID NOT LIKE ('ADM03','ADM04','ADM01','ADM02','ADM00','ADM05','ADMPC01','XPENDA000000002','XPENDA000000001','XPENDA000000003')
    //         -- AND   TO_CHAR(A.CREATED_DATE,'YYYY')='2026'
    //         AND   A.CREATED_BY = ?
    //         -- AND   A.CREATED_BY ='DR. S0000000007'
    ...
    //         AND A.LAYAN_ID IS NOT NULL 
    //         AND A.LAYAN_ID NOT LIKE ('ADM03','ADM04','ADM01','ADM02','ADM00','ADM05','ADMPC01','XPENDA000000002','XPENDA000000001','XPENDA000000003')
    //         -- AND   A.ANS_DOKTER_ID ='DR. S0000000007'
    //         AND   A.ANS_DOKTER_ID = ?
    ...
    //         -- AND   TO_CHAR(A.CREATED_DATE,'YYYY')='2026'
    //         AND   A.ANK_DOKTER_ID= ?
    //         -- AND   A.ANK_DOKTER_ID='DR. S0000000007'
    //         GROUP BY ANK_DOKTER_ID, LAYAN_ID
    //     )X
    //     ORDER BY X.TOTAL_QTY DESC, NAMADOKTER, JENIS, NAMAPELAYANAN";
    //     // -- ORDER BY NAMADOKTER, JENIS, NAMAPELAYANAN";

    //     // Urutan parameter binding (?) dikirim 3 kali untuk 3 blok UNION
    //     $bind_params = array(
